...
const paragraph = NotionHelper.block.paragraph.createBlock(
  "My mistake. Table's cold, anyway."
);

// Usage example with shorthand function
const paragraph = NotionHelper.paragraph(
  "I'd try the lounge at the Caesar's. It gets busy after three o'clock."
);
```
...
Accounting for all of these limitations in production code is hard. Doing it in a way that _minimizes API requests_ (and thus maximizes performance) is hard enough that you basically need to build an entire library for it... hence Notion Helper.
...
**Note**: When using `createPage()`, you'll probably want to include _both_ of these functions. When you provide body content that needs to be split into multiple chunks, `createPage()` will handle the first one and then kick all the others over to `appendBlocks()` automatically.
...
const builder = createNotionBuilder()
  ...
  .date("Due", "2025-10-29")
  .paragraph(
    "I need to walk Mrs. Nesbit's dog today. The leash is hanging near the door."
  )
  .build();
...
This sets the builder up to allow any number of blocks in _any_ `children` array (even nested ones), and allows for an infinite number of nested `children` levels.
...
- Position (for pages with a page parent)
    ...
    text: [
      {
        ...
        text: {
          content:
            "This is a custom paragraph block, added via addExistingBlock().",
        },
      },
    ],
  },
...
_Note: Using `"default"` when a data source has no default template will cause a Create Page request to fail. The "Empty" option is not considered a default with respect to the API, even though it does show the "Default" badge in the Notion app._
...
async function verifyTemplateReady({
  ...
  const { maxRetries = 3, waitMs = 2000 } = options;
  ...
  if (
    ...
    throw new Error(
      'Template verification failed: template object is invalid or missing required "type" property'
    );
  }
  ...
  if (template.type === "template_id") {
    ...
    if (!templateId) {
      throw new Error(
        'Template verification failed: Missing template_id for template.type "template_id"'
      );
    }
    ...
    try {
      ...
      if (defaultTemplate) {
        ...
        if (!templateId) {
          throw new Error(
            "Template verification failed: Default template found but it has no id"
          );
        }
        console.log(
          `   Found default template: ${defaultTemplate.name} (${templateId})`
        );
      } else {
        throw new Error(
          "Template verification failed: No default template found in the provided data source"
        );
      }
    } catch (error) {
      throw new Error(
        `Template verification failed: Could not fetch templates (${error.message})`
      );
    }
  } else if (template.type === "none") {
    throw new Error(
      'Template verification failed: Template type is "none", nothing to verify.'
    );
  } else {
    throw new Error(
      `Template verification failed: Unknown template type "${template.type}"`
    );
  }
  ...
  try {
    ...
    throw new Error(
      `Template verification failed: Could not fetch template blocks (${error.message})`
    );
  }
  ...
  while (retries <= maxRetries && !verified) {
    ...
    try {
      ...
      throw new Error(
        `Template verification failed: Could not fetch page blocks for verification attempt ${
          retries + 1
        } (${error.message})`
      );
    }
    ...
    console.log(
      `   Attempt ${retries + 1}/${
        maxRetries + 1
      }: Page has ${currentBlockCount} blocks`
    );
...
| `block.breadcrumb.createBlock()`         | _(no shorthand)_                   | [Breadcrumb](https://developers.notion.com/reference/block#breadcrumb)                         |
...
// External URL (string)
const block1 = audio(
  "https://thomasjfrank.com/wp-content/uploads/2025/11/Voice-Sample3.mp3"
);
...
// External URL (string)
const block1 = file(
  "https://collegeinfogeek.com/wp-content/uploads/2015/01/10steps-reddit.pdf"
);
...
const block5 = heading2({
  ...
  children: [paragraph("Somebody made a duplicate of my vault.")],
});
```
...
// Single string
const block1 = heading3(
  "I still owe you for the thing with the guy in the place, and I'll never forget it."
);

// Array of strings
const block2 = heading3([
  "You could try the lounge at the Caesar's. ",
  "I hear it gets busy after three o'clock.",
]);
...
const block5 = heading3({
  ...
  is_toggleable: true,
  children: [paragraph("All right chaps. Hang on to your knickers.")],
});
```
...
const block6 = paragraph(
  [
    "What, did you guys get a group rate or something? ",
    buildRichTextObj("All right chaps. Hang on to your knickers.", {
      italic: true,
    }),
  ].flat()
);
...
_To set data source property values, see the Page Property Methods below to explore Notion Helper's `page_props` API._
...
const pageParent = parentPage("your-page-id"); // If parent is a page
...
const position = position("page_start"); // Only for pages with page parent
```
...
| `page_meta.template.createMeta()` | _(used via `template()` in builder)_ | Creates a template object                     |
| `page_meta.position.createMeta()` | `position()`                         | Creates a position object (page parent only)  |
...
When creating a page with a page parent (not a data source or database), you can specify where the new child page should be placed within the parent page using the `position` parameter.

This is useful for controlling the order of sub-pages, or placing a page after a specific block in the parent page.

```js
import { page_meta, position, createNotionBuilder } from "notion-helper";

// Using page_meta API - place at page start
const position1 = page_meta.position.createMeta("page_start");

// Using shorthand alias - place at page end (default behavior)
const position2 = position("page_end");

// Place after a specific block (using block ID)
const position3 = position("your-block-id-here");

// Using string shortcuts
const position4 = position("start"); // Same as "page_start"
const position5 = position("top"); // Same as "page_start"
const position6 = position("end"); // Same as "page_end"
const position7 = position("bottom"); // Same as "page_end"

// Using a fully-formed position object
const position8 = position({
  type: "after_block",
  after_block: { id: "your-block-id-here" },
});

// Using in the fluent builder
const page = createNotionBuilder()
  .parentPage("parent-page-id")
  .position("page_start") // Place at top of parent page
  .title("title", "My Sub-Page")
  .paragraph("This page will appear at the top of the parent.")
  .build();

// Place after a specific block in the parent page
const page2 = createNotionBuilder()
  .parentPage("parent-page-id")
  .position("specific-block-id")
  .title("title", "Another Sub-Page")
  .build();
```

**Note:** The `position` parameter is only valid when the parent is a page. It cannot be used when creating pages under a data source or database.
...
const richTextProp3 = richText([
  "This is rich text content with ",
  "**formatting**",
  " and emojis 🤔",
]);

// Using buildRichTextObj() with annotations
const richTextProp4 = richText(buildRichTextObj("Bold text", { bold: true }));
...
// Single URL string
const filesProp = page_props.files.setProp(
  "https://collegeinfogeek.com/wp-content/uploads/2015/01/10steps-reddit.pdf"
);
const filesProp2 = files(
  "https://collegeinfogeek.com/wp-content/uploads/2015/01/10steps-reddit.pdf"
);

// Array of URLs
const filesProp3 = files([
...
const multiSelectProp = page_props.multi_select.setProp([
  "Option 1",
  "Option 2",
]);
...
const peopleProp = page_props.people.setProp([
  "01b92c2b5a38495da82b922f47b3a308",
]);
...
const relationProp = page_props.relation.setProp([
  "01b92c2b5a38495da82b922f47b3a308",
]);
...
// Simple string
const richText1 = buildRichTextObj(
  "You're both of you nuts. I know more about casino security than any man alive. I invented it!"
);
...
// With hyperlink
const richText4 = buildRichTextObj(
  "I still owe you for the thing with the guy in the place, and I'll never forget it.",
  {
...
const richText5 = buildRichTextObj(
  "You could try the lounge at the Caesar's. I hear it gets busy after three o'clock.",
  {
    annotations: {
      bold: true,
...
const longBlock = paragraph(
  "This is a very long paragraph that exceeds the character limit..."
);
...
const url2 =
  "https://www.notion.so/My-Page-12345678-90ab-cdef-1234-567890abcdef";
